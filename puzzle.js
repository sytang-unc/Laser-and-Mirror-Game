...
var sol = [orientEnum.CONS, orientEnum.REV, orientEnum.CONS, orientEnum.REV]; 
var clues = [
...
function createHiddenAndClues(puzzle){
	if (puzzle.level == 0){
		puzzle.clues[0] = "There are 0 hidden mirrors";
		return;
	}
	var usedGrid = new Array(puzzle.gridSize);
	var i,j,k;
	for (i = 0; i < puzzle.gridSize; i++){
		usedGrid[i] = new Array(puzzle.gridSize);
		for(j = 0; j < puzzle.gridSize; j++){
			usedGrid[i][j] = 0;
		}
	}
	...
	while( !puzzle.pathGrid[i][j] );
	var positions = new Array(puzzle.level + 2);
	positions[0] = new coord(i,j);
	usedGrid[i][j] = 1;
	for(k = 1; k < puzzle.level + 2; k++){
		do{
			i = rand(0, puzzle.gridSize - 1);
			j = rand(0, puzzle.gridSize - 1);
		}
		while (usedGrid[i][j] || puzzle.grid[i][j]);
		positions[k] = new coord(i,j);
		usedGrid[i][j] = 1;
	}
	shuffle(positions, puzzle.level + 2);
	for(i = 0; i < puzzle.level; i++){
		console.log("sol[i]: " + sol[i]);
		puzzle.grid[positions[i+2].x][positions[i+2].y] = new bumper(sol[i], 0, 1);
	}
	var varSubset = new Array(puzzle.level + 2);
	for(i = 0; i < puzzle.level + 2; i++){
		varSubset[i] = variables[i];
	}
	shuffle(varSubset, puzzle.level + 2);
	for(i = 0; i < puzzle.level + 2; i++){
		puzzle.clueGrid[positions[i].x][positions[i].y] = varSubset[i];
	} 
	for(i = 0; i < 2*puzzle.level; i++){
		puzzle.clues[i] = substitute(clues[i], varSubset, 2 + puzzle.level);
	}
	puzzle.clues[2*puzzle.level] = "There are " + puzzle.level + " hidden mirrors";
	shuffle(puzzle.clues, 2*puzzle.level + 1);
}

function substitute(clue, varArray, num){
